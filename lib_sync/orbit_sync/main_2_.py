...
class RealtimeApp(App[None]):
    ...
    async def handle_realtime_connection(self) -> None:
        while True:
            try: 
                async with self.client.beta.realtime.connect(model="gpt-4o-mini-realtime-preview-2024-12-17" , websocket_connection_options={"ping_interval" : 19 , "ping_timeout" : 60} , ) as conn:
                        ...
                        async for event in conn:
                            ...
                            if event.type == "conversation.item.input_audio_transcription.completed":
                                ...
                                if "bye" in event.transcript.lower():
                                    ...
                                    return
                            if event.type == 'error':
                                logging.error(f"Error Type: {event.error.type}")
                                logging.error(f"Error Code: {event.error.code}")
                                logging.error(f"Event ID: {event.error.event_id}")
                                logging.error(f"Error Message: {event.error.message}")
                                continue
